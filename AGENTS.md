...
3. Review linked pull requests, issues, Linear work, related repositories in `scintilla-run`, and relevant external-organization repositories whenever behavior or contracts cross boundaries.
...
For synchronization, avoid git rebase in favor of git merge so shared history remains append-only and independently auditable.
